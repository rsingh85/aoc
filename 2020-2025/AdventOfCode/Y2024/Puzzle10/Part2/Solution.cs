namespace AdventOfCode.Y2024.Puzzle10.Part2
{
    public class Solution : ISolution
    {
        ...
        private static HashSet<string> uniquePathsReachingTarget = new();

        private static int FindPaths((int r, int c) trailhead, (int r, int c) currentCoords, char[,] grid, List<string> path)
        {
            var current = grid[currentCoords.r, currentCoords.c];
            path.Add($"({currentCoords.r},{currentCoords.c})");

            if (current == '9')
            {
                if (uniquePathsReachingTarget.Add(string.Join(string.Empty, path)))
                    return 1;
