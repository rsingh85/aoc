namespace AdventOfCode.Y2024.Puzzle8.Part2
{
        ...
        private int AddAntiNodes(char[,] grid, Dictionary<char, List<(int r, int c)>> antennaLocations)
        {
            ...
            foreach (var antenna in antennaLocations.Keys)
            {
                ...
                for (var i = 0; i < locations.Count; i++)
                {
                    ...
                    {
                        var currentLocationAntiNodeR = currentLocation.r;
                        var currentLocationAntiNodeC = currentLocation.c;

                        while (IsLocationInBounds(currentLocationAntiNodeR, currentLocationAntiNodeC, grid.GetLength(0), grid.GetLength(1)))
                        {
                            if (grid[currentLocationAntiNodeR, currentLocationAntiNodeC] == '.')
                            ...
                            currentLocationAntiNodeR = currentLocationAntiNodeR - (otherLocation.r - currentLocation.r);
                            currentLocationAntiNodeC = currentLocationAntiNodeC - (otherLocation.c - currentLocation.c);
                        }
                    }
                }
            }
