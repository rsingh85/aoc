...
const unusedDiskSpace = totalDiskSpace - dirMap.get('/').getTotalFileSize();
const minimumDiskSpaceToFreeUp = requiredDiskSpace - unusedDiskSpace;

const result =
  [...dirMap.keys()]
      .map((k) => dirMap.get(k))
      .map((d) => d.getTotalFileSize())
      .filter((s) => s >= minimumDiskSpaceToFreeUp);
